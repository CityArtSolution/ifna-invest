...
class ins_account_financial_report(models.Model):
    ...
    def _get_children_by_order(self, strict_range):
        ...
        if not strict_range:
            res -= self.env.ref('account_dynamic_edited.ins_account_financial_report_unallocated_earnings0')
            res -= self.env.ref('account_dynamic_edited.ins_account_financial_report_equitysum0')
        return res
    ...
    account_ids = fields.Many2many('account.account', 'ins_account_account_financial_report', 'report_line_id',
                                   'account_id', 'Accounts')
    account_report_id = fields.Many2one('ins.account.financial.report', 'Report Value')
    account_type_ids = fields.Many2many('account.account.type', 'ins_account_account_financial_report_type',
...
class AccountAccount(models.Model):
    ...
    def get_cashflow_domain(self):
        cash_flow_id = self.env.ref('account_dynamic_edited.ins_account_financial_report_cash_flow0')
        if cash_flow_id:
            return [('parent_id.id', '=', cash_flow_id.id)]

    cash_flow_category = fields.Many2one('ins.account.financial.report', string="Cash Flow type",
    ...
    def onchange_cash_flow_category(self):
        ...
        if self._origin and self._origin.id:
            ...
            self.env.ref(
                'account_dynamic_edited.ins_account_financial_report_cash_flow0').write(
