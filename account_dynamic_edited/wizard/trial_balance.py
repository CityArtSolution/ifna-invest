...
class InsTrialBalance(models.TransientModel):
    ...
    group_level_id = fields.Many2one(
        'account.group.level', string='Group Level'
    )
    ...
    # @api.onchange('group_level_id')
    # def _onchange_group_level_id(self):
    #     for rec in self:
    #         if rec.group_level_id:
    #             level_ids = []
    #             level_ids.append(rec.group_level_id.id)
    #             level_child = self.env['account.group.level'].search(
    #                 [('parent_id', '=', rec.group_level_id.id)])
    #             while level_child:
    #                 level_ids.append(level_child.id)
    #                 level_child = self.env['account.group.level'].search(
    #                     [('parent_id', '=', level_child.id)])
    #             account_ids = self.env['account.account'].search(
    #                 [('group_id.level_id', 'in', level_ids)])
    #             if account_ids:
    #                 rec.account_ids = account_ids.ids
    ...
    def print_xls(self):
        ...
        return self.env.ref('account_dynamic_edited.action_ins_trial_balance_xlsx').report_action(self, data=data)
    ...
    def process_filters(self, data):
        ...
        if data.get('group_level', False):
            filters['group_level'] = True
        else:
            filters['group_level'] = False
    ...
    def process_data(self, data):
        if not self.group_level:
            if data:
                ...
                # if self.group_level_id:
                #     account_ids = self.env['account.account'].search([('group_id.level_id', '=', self.group_level_id.id)])
            ...
            all_total_cre = 0.0
            all_total_bln = 0.0
            all_total_init_deb = 0.0
            all_total_init_cre = 0.0
            all_total_init_bal = 0.0
            group_ids = self.env['account.group'].search(
                [('level_id', '=', self.group_level_id.id)])
            move_lines = {x.name: {'name': x.name, 'code': "", 'id': x.id,
                                   'initial_debit': 0.0, 'initial_credit': 0.0, 'initial_balance': 0.0,
                                   'debit': 0.0, 'credit': 0.0, 'balance': 0.0,
                                   'ending_credit': 0.0, 'ending_debit': 0.0, 'ending_balance': 0.0,
                                   'company_currency_id': company_currency_id.id} for x in
            ...
            for group in group_ids:
                ...
                total_init_bal = 0.0
                group_child_ids = []
                group_child_ids.append(group.id)
                group_child = self.env['account.group'].search(
                    [('parent_id', '=', group.id)])

                while len(group_child) != 0:
                    group_child_ids += group_child.ids
                    group_child = self.env['account.group'].search(
                        [('parent_id', 'in', group_child.ids)])
                group_child_obj_ids = self.env['account.group'].search(
                    [('id', 'in', group_child_ids)])
                account_ids = self.env['account.account'].search(
                    [('group_id', 'in', group_child_obj_ids.ids)])
                ...
                for account in account_ids:
                    ...
                    #         continue
                    #     else:
                    #         move_lines.pop(group.name)
                    # else:
                    total_deb += deb
                    total_cre += cre
                    total_bln += bln
                all_total_init_deb += total_init_deb
                all_total_init_cre += total_init_cre
                all_total_init_bal += total_init_bal
                all_total_deb += total_deb
                all_total_cre += total_cre
                all_total_bln += total_bln
                move_lines[group.name]['debit'] = total_deb
                move_lines[group.name]['credit'] = total_cre
                move_lines[group.name]['balance'] = total_bln
                move_lines[group.name]['ending_balance'] = company_currency_id.round(total_init_bal + total_bln)
                move_lines[group.name]['ending_credit'] = company_currency_id.round(total_init_cre + total_cre)
                move_lines[group.name]['ending_debit'] = company_currency_id.round(total_init_deb + total_deb)
            subtotal = {'SUBTOTAL': {
                'name': 'Total',
                'code': '',
                'id': 'SUB',
                'initial_credit': company_currency_id.round(all_total_init_cre),
                'initial_debit': company_currency_id.round(all_total_init_deb),
                'initial_balance': company_currency_id.round(all_total_init_bal),
                'credit': company_currency_id.round(all_total_cre),
                'debit': company_currency_id.round(all_total_deb),
                'balance': company_currency_id.round(all_total_bln),
                'ending_credit': company_currency_id.round(all_total_init_cre + all_total_cre),
                'ending_debit': company_currency_id.round(all_total_init_deb + all_total_deb),
                'ending_balance': company_currency_id.round(all_total_init_bal + all_total_bln),
                'company_currency_id': company_currency_id.id}}
