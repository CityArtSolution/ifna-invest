...
class InsGeneralLedger(models.TransientModel):
    _name = "report.ins.general.ledger"
    try:
    ...
    def build_detailed_move_lines(self, offset=0, account=0, fetch_range=FETCH_RANGE):
        ...
        if ((count - offset_count) <= fetch_range) and data.get('initial_balance'):
            ...
            for row in cr.dictfetchall():
                ...
                x = {}
                x['move_name'] = 'Ending Balance'
                x['account_id'] = account
                x['company_currency_id'] = currency_id.id
                x['debit'] = debit
                x['credit'] = credit
                x['balance'] = balance
                move_lines.append(x)
        ...
        for account in account_ids:
            ...
            for row in cr.dictfetchall():
                if data.get('display_accounts') == 'balance_not_zero' and currency.is_zero(
                    ...
                    count, offset, sub_lines = self.build_detailed_move_lines(offset=0, account=account.id,
                                                                              fetch_range=1000000)
                    debit = 0
                    credit = 0
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Ending Balance':
                    ...
                    move_lines[account.code]['lines'].append(row)
                    move_lines[account.code]['debit'] = debit
                    move_lines[account.code]['credit'] = credit
                    move_lines[account.code]['balance'] = balance
    ...
    def print_xls(self):
        ...
        data = {'from_date': date_from, 'to_date': date_to, 'company_id': self.company_id}
        return self.env.ref('account_dynamic_edited.action_ins_general_ledger_xlsx').report_action(self, data=data)
