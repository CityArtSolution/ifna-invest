...
class InsPartnerLedger(models.TransientModel):
    ...
    def process_filters(self):
        ...
        if data.get('units_ids', []):
            filters['units'] = self.env['operating.unit'].browse(
    ...
    def build_detailed_move_lines(self, offset=0, partner=0, analytic_account=0, fetch_range=FETCH_RANGE):
        ...
        # WHERE_CURRENT += " AND pp.analytic_account = %s" analytic.account
        if data.get('initial_balance'):
        ...
        # WHERE_FULL += " AND pp.analytic_account = %s" analytic.account
        ...
        sql = ('''
                SELECT
                    ...
                    l.operating_unit_id AS operating_unit,
...
class PartnerReportWizardController(http.Controller):
    def convert_to_date(self, datestring=False):
        if datestring:
            ...
            language_id = request.env['res.lang'].search(
                [('code', '=', lang)])[0]
            datestring = fields.Date.from_string(
                datestring).strftime(language_id.date_format)
    ...
    def export_partner_report(self, wizard_id):
        ...
        if record:
            ...
            sheet.merge_range(0, 0, 0, 8, 'Partner Ledger' +
                              ' - ' + data[0]['company_id'][1], format_title)
            filter, acc_lines = record.get_report_datas()
            if len(filter.get('partners')) > 0:
                p_list = ', '.join([lt or '' for lt in filter.get('partners')])
                sheet.merge_range(1, 2, 1, 5, 'Customer: ' +
                                  p_list, format_title)
            sheet.write_string(1, 0, _('Date from'),
                               format_header)
            sheet.write_datetime(1, 1, self.convert_to_date(str(filter['date_from']) or ''),
                                 content_header_date)
            sheet.write_string(2, 0, _('Date to'),
                               format_header)
            sheet.write_datetime(2, 1, self.convert_to_date(str(filter['date_to']) or ''),
                                 content_header_date)
            # Filter section
            row_pos_2 = 0
            row_pos_2 += 2
            if filter:
                ...
                sheet_2.write_string(row_pos_2, 0, _('Partner Tag'),
                                     format_header)
                p_list = ', '.join(
                    [lt or '' for lt in filter.get('categories')])
                sheet_2.write_string(row_pos_2, 1, p_list,
                                     content_header)
                    ...
                    if filter.get('include_details', False):
                        ...
                        for sub_line in sub_lines:
                            ...
                            if sub_line.get('move_name') == 'Initial Balance':
                                ...
                                sheet.write_string(
                                    row_pos, 0, acc_lines[line].get('name'), line_header)
                                if sub_line.get('operating_unit'):
                                    operating_unit = record.env['operating.unit'].browse(
