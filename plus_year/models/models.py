...
class RentSaleOrder(models.Model):
    ...
    def test(self):
        ...
        for x in range(1, (self.years_number) + 1):
            nums.append(x)
        selection = [(str(num), str(num)) for num in nums]
        print("////////////////////////////////////", selection)
        return selection

    year_number = fields.Selection(string="رقم السنة", selection=lambda self:self.test(), default="")
...
class RentSaleOrderLine(models.Model):
    ...
    # @api.depends('order_id.years_number')
    # def test(self):
