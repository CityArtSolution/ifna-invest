...
def amount_to_text_arabic(number, currency):
    ...
    cents_name = (cents_number > 10 or cents_number == 0) and _get_currency_name_by_code(currency)[2] or \
                 _get_currency_name_by_code(currency)[1]

    if int(list[1]) > 0:
        return u'فقط ' + start_word + ' ' + _get_currency_name_by_code(currency)[
            0] + ' ' + u'و' + ' ' + end_word + " " + cents_name
    else:
